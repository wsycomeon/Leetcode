...
public class MainDp {
     ...
     * <p>
     * todo 而 物品 如果是 主件 ，又 最多 俩附件的话（2 x 2 = 4），就不是 两种情况，而是 1 + 4 = 5种 情况：
     * <p>
     * 不选 --》dp[i-1][j]
    ...
    public static void main(String[] args) {
        ...
        for (int i = 1; i < m + 1; i++) {
            ...
            for (int j = 1; j < n + 1; j++) {
                if (good.q > 0) {
                    // todo 当前是 附件，则 不考虑，即 dp值 不变 --》只用 主件 来 统一 考虑
                    dp[i][j] = dp[i - 1][j];

                } else {
                    // todo 当前是 主件，则考虑 5种情况的 最大值
                    // 1、不选 主件
                    int max = dp[i - 1][j];
                    // 2、只选 主件
                    if (cost0 <= j) {
